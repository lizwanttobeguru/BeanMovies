<template>
  ...
  <view class="">
    ...
    <view class="width-000 " v-if="!item.searchVal">
      <commonTitle :defaultSty="false" :isShowIcon="true" :right="true" :title="item.type || item.searchVal">
      </commonTitle>
    </view>
    <!-- 导航栏 -->
    <!-- 搜索框 -->

    <view v-else class="header-nav-box" style="position: relative;"
      :style="'height:'+navHeight+'px; line-height: '+navHeightLine+'px'">
      <view @tap="backTap" class=" iconfont iconyoujiantou1 font-weight-700 position-absolute"
        style="left: 10rpx; font-size: 45rpx;color: #fff;">

      </view>
      <view class="search-cont px-3 ml-3">

        <view class="position-relative">
          <input @confirm="search(searchVal)"
            style="height: ; padding-left: 66rpx;box-sizing: border-box; background: #FFFFFF; border-radius: 50rpx;"
            confirm-type="search" v-model="searchVal" placeholder="请输入关键字" />
          <text @tap="search(searchVal)" style="top: 50%;transform: translateY(-50%);left: 20rpx"
            class="position-absolute iconfont iconsousuo1"></text>
        </view>
