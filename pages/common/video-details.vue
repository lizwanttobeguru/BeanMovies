<template>
  ...
  <view class="">
    ...
    <view class="width-000 ">
      <commonTitle @backTap="backTap" :defaultSty="false" :isShowIcon="true" :themeBg="false" :right="true"
        :title="item.vod_name"></commonTitle>
    </view>
    <!-- 导航栏 -->
    ...
    <view class="position-relative" style="color: #fff;">
      <image @error="imgerror" :src="videoDatails.vod_pic"
        style="height: 622rpx; top: -200rpx; position: absolute; width: 100%; filter: blur(30px) brightness(70%);color: #fff;"
        mode=""></image>
      <view class="" style="position: absolute;left: 0; width: 100%;">
        <view class="d-flex a-center mx-4 ">
          ...
          <view class="" style="width: 60%;">
            ...
            <view class="font-27 mb-2">
              {{filterVoList.year || '未知'}} / {{videoDatails.vod_area || '未知'}} /
              {{filterVoList.directors[0].name || '未知' }}
              <!--  -->
              <!---->
            </view>
            <view class="d-flex a-center  operate">
              <view class="mr-2 d-flex a-center">
                ...
                <!-- #ifdef MP-WEIXIN -->
                <button data-name="shareBtn" class="d-flex a-center j-center" open-type="share"
                  style="height: 60rpx; background: rgb(244, 156, 54); width: 100%;text-align: left; padding: 0px;position: none;font-size: 27rpx;">
                ...
                <!-- #endif -->
                <!-- #ifdef APP-PLUS -->
                <button @tap="invite" class="d-flex a-center j-center"
                  style="height: 60rpx; background: rgb(244, 156, 54); width: 100%;text-align: left; padding: 0px;position: none;font-size: 27rpx;">
        ...
        <view class="mt-5 px-2 d-flex j-sb a-center mx-2 theme-border-r10"
          style="height: 150rpx;background: rgb(148, 169, 191);">
          <view v-show="filterVoList.rating.value" class="d-flex a-center font-weight-600 font-33 "
            style="color: rgb(244, 209, 119);">
            <view class="mr-2 font-weight-600 font-33 ">
              评分：{{filterVoList.rating.value}}
            </view>
            <view class="font-weight-600 font-33 d-flex a-center">
              <view :class="item == 3?'font-36 iconai65':item == 1?'font-40 iconwujiaoxing1':'iconwujiaoxing3'"
                class="font-33 iconfont " v-for="(item,index) in newArr" :key="index">
          ...
          <view class="d-flex a-center" @tap="toKan"
            v-if="this.item.type_id && !!this.videoDatails.vod_tab && this.videoDatails.vod_pu != 1">
    ...
    <view class="mx-2 mt-3">
      ...
      <view class="d-flex j-center mt-1" v-if="videoDatails.vod_content.length > 100" @tap="isShowEvent">
        <view class="d-flex a-center d-flex a-center">
          ...
          <view class="iconfont " :class="isShow?'iconshangjiantou':'iconxiajiantou1'"
